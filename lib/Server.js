...
class MeshbluServer {
  ...
  cleanupRedis(callback) {
    ...
    const webhookNamespace = this.MESHBLU_OPTIONS.webhookWorker.namespace
    const firehoseNamespace = this.MESHBLU_OPTIONS.meshbluFirehose.namespace
    ...
    async.parallel([
      ...
      (next) => {
        this.redis.keys(`${webhookNamespace}*`, (error, keys) => {
          if (error) return next(error)
      ...
      (next) => {
        this.redis.keys(`${firehoseNamespace}*`, (error, keys) => {
          if (error) return next(error)
